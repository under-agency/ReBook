...
- Вопросы о свободном времени и мастерах («Маша свободна в субботу?», «к Лене можно?», «есть окошко завтра?») — это intent=book, а не question.
- service_id — только id из списка услуг. Если фраза подходит к нескольким услугам сразу («подстричься» при женской и мужской стрижке) — service_id=null и service_mentioned=null, бот сам уточнит. service_mentioned заполняй, только если клиент назвал услугу, которой точно нет в списке («педикюр» при отсутствии педикюра).
...
- Время — в часах работы салона: «после 6» / «после шести» при работе до 20:00 значит 18:00, «в 3» — 15:00. «После X» → time_from=X, time_to=null. «До X» → time_to=X. «Утром» → 10:00–12:00, «днём» → 12:00–17:00, «вечером» → 17:00–время закрытия. Точное время («в 15:30», «в 19», «в 3», «на 12») → time_from=time_to, например «в 19» → time_from="19:00", time_to="19:00". Не сказал про время — оба null.
...
def parse(db: Session, salon: Salon, safe_text: str, context: str | None = None) -> Draft:
    ...
    try:
        data = llm.parse_json(llm.complete(messages))
    except llm.LLMError:
        # провайдер изредка обрывает ответ или отвечает с задержкой — один повтор
        data = llm.parse_json(llm.complete(messages))
    return _to_draft(data, salon, services, staff)
